...
tb_cfg = TB_CONFIG["tb3_bg"]

grid = Grid(
    np.array([-2.5, -2.5, -np.pi]),  # meters, meters, radians
    np.array([2.5, 2.5, np.pi]),  # meters, meters, radians
    3,
...
turtlebot3_bg_no_model_mismatch = DubinsCar(
    r=tb_cfg['RADIUS'],
    uMode="max",
    dMode="min",
    speed=tb_cfg['SPEED'],
...
turtlebot3_bg_model_mismatch = DubinsCar(
    r=tb_cfg['RADIUS'],
    uMode="max",
    dMode="min",
    speed=tb_cfg['SPEED'],
...
brt_no_model_mismatch_file = "turtlebot_3_brt_speed_01_wMax_11_dstb.npy"
brt_model_mismatch_file = "turtlebot_3_brt_speed_01_wMax_07_dstb.npy"
...
if __name__ in "__main__":
    ...
    result = HJSolver(
        turtlebot3_bg_no_model_mismatch,
        grid,
        ivf,
        tau,
        compMethods,
    ...
    po2 = PlotOptions(
        do_plot=False, plot_type="set",
        plotDims=[0,1,2],
        slicesCut=[], save_fig=True,
        filename="my_test.png"
        )
    visualize_plots(result, grid, po2)

    result = HJSolver(
        turtlebot3_bg_model_mismatch,
        grid,
        ivf,
        tau,
        compMethods,
